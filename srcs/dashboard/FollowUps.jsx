...
// import { fetchLeadsByFollowUpDate, resetSearchLeadsState } from '../redux/slice/index';
...
const FollowUps = ({ route }) => {
  ...
  const [lastQueryType, setLastQueryType] = useState(null); // Local state to track last queryType
  const [modalVisible, setModalVisible] = useState(false);
  const [modalContent, setModalContent] = useState({ title: '', content: '' });
  const [assignModalVisible, setAssignModalVisible] = useState(false);
  const [selectedLeadId, setSelectedLeadId] = useState(null);
  ...
  useEffect(() => {
    // Fetch data only if queryType has changed or no leads exist
    if (lastQueryType !== queryType || leads.length === 0) {
      dispatch(resetSearchLeadsState()); // Reset state for new queryType
      dispatch(fetchLeadsByFollowUpDate({ queryType, page: 1, pageSize: 10 }));
      setLastQueryType(queryType); // Update lastQueryType
    }
  }, [dispatch, queryType, lastQueryType, leads.length]);
            ...
            <View className="flex-row">
              <View className="w-[195] flex-row items-center gap-x-6">
                <Image
                  ...
                  resizeMode="contain"
                />
                <StyledText className="text-[#000] text-inputs font Facilement
                font-familyFontInter">
                  {item.assignedTo || 'N/A'}
                </StyledText>
              </View>
              <View className="w-1/2 flex-row items-center gap-x-2">
  ...
  const getHeaderTitle = () => {
    // Dynamic header title based on queryType
    return `${queryType.charAt(0).toUpperCase() + queryType.slice(1)} FollowUps Detail`;
  };
