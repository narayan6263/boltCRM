...
const EditLeadsDetails = ({ route, navigation }) => {
  ...
  useEffect(() => {
    const fetchDecryptedContactInfo = async () => {
      if (lead?.contactId) {
        try {
          //.log('EditLeadsDetails: Fetching decrypted contact info for contactId:', lead.contactId);
          await dispatch(getDecryptInfo(lead.contactId)).unwrap();
        } catch (error) {
          //.error('EditLeadsDetails: Error fetching decrypted contact info:', error);
          //.warn('EditLeadsDetails: Falling back to encrypted values due to decryption failure');
  ...
  useEffect(() => {
    if (decryptedData && decryptStatus === 'succeeded') {
      //.log('EditLeadsDetails: Updating form with decrypted data:', decryptedData);
      ...
      if (decryptedPhone) {
        ...
        //.log('EditLeadsDetails: Updated mobile number with decrypted value:', decryptedPhone);
      }
      
      if (decryptedEmail) {
        setEmail(decryptedEmail);
        //.log('EditLeadsDetails: Updated email with decrypted value:', decryptedEmail);
      }
    }
