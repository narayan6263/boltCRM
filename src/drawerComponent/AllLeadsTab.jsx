...
const AllLeadsTab = ({ navigation, onLeadAdded }) => {
  ...
  const { formattedLeads = [], loading, error, hasMore, currentPage, totalLeads } = useSelector(
    (state) => state.leads.allLeads
  );
  ...
  const slideAnim = useRef(new Animated.Value(-Dimensions.get('window').height * 0.65)).current;
        ...
        >
          ...
          <View>
            <StyledText className="text-subHeading font-poppinsFont text-black">
              Total Lead: <Text> {totalLeads || 0}</Text>
            </StyledText>
