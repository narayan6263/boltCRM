...
// const HistoryTab = ({ lead, navigation, isActive }) => {
...
//   useEffect(() => {
//     if (!isActive) {
//       // Reset or clear data when tab is not active to prevent stale data display
//       // dispatch(resetFollowUpsState()); // Assuming you have a reset action for follow ups
//       return;
//     }
...
//     // Only fetch if data hasn't been fetched for this leadId yet or if we need to refetch on activation
...
const HistoryTab = ({ lead, navigation, isActive }) => {
  ...
  useEffect(() => {
    if (!isActive) {
      console.log('HistoryTab: Tab inactive, skipping fetch');
      return;
    }
    ...
    if (!hasFetched.current[lead.leadId]) {
      console.log(`HistoryTab: Fetching follow-ups for leadId ${lead.leadId}`);
      dispatch(fetchLeadFollowUps(lead.leadId))
        .then(() => {
          hasFetched.current[lead.leadId] = true; // Mark as fetched
        })
        .catch((err) => console.error('HistoryTab: Dispatch error:', err));
    } else {
      console.log(`HistoryTab: Skipping fetch for leadId ${lead.leadId}, using cached data`);
    }
  }, [dispatch, lead?.leadId, isActive]);
