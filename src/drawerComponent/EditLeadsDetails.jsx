...
// import { getDecryptInfo, resetDecryptInfo,fetchLeadById,updateLeadAsync,updateContact,fetchAllBranches,fetchAllSources } from '../redux/slice/index';
...
//   const [mobileNo, setMobileNo] = useState(lead?.phone || '');
...
//   // Effect to handle decrypted data
//   useEffect(() => {
//     if (decryptStatus === 'success' && decryptedData?.data) {
//       if (decryptedData.data.phone) {
//         setMobileNo(decryptedData.data.phone);
//       }
//       if (decryptedData.data.email) {
//         setEmail(decryptedData.data.email);
//       }
//     }
//   }, [decryptStatus, decryptedData]);

//   // Log renders for debugging
//   useEffect(() => {
//     renderCount.current += 1;
//     //.log(`EditLeadsDetails render count: ${renderCount.current}`);
//     //.log('EditLeadsDetails: route.params.lead', JSON.stringify(lead, null, 2));
//     //.log('EditLeadsDetails: address state', JSON.stringify(address, null, 2));
//   }, [lead, address]);

//   // Fetch sources and branches when the screen focuses
//   useFocusEffect(
//     useCallback(() => {
...
//       //.error('EditLeadsDetails: Invalid Lead or Contact ID', { leadId: lead?.leadId, contactId: lead?.contactId });
//       Alert.alert('Error', 'Invalid Lead or Contact ID.');
//       return;
//     }
//     if (!source || !sourceId || !isValidObjectId(sourceId)) {
//       //.error('EditLeadsDetails: Invalid source', { source, sourceId });
//       Alert.alert('Error', 'Please select a valid source.');
//       return;
//     }
//     if (branchId && !isValidObjectId(branchId)) {
//       //.error('EditLeadsDetails: Invalid branchId', { branchId });
...
//       //.error('EditLeadsDetails: Validation failed', { referenceError, descriptionError, commentError, modelError });
//       Alert.alert('Error', 'Please fix the validation errors before submitting.');
//       return;
//     }
//     if (!validateEmail(email)) {
//       //.error('EditLeadsDetails: Invalid email', { email });
...
//       //.log('EditLeadsDetails: fetchResult.data', JSON.stringify(fetchResult.data, null, 2));
...
//       //.log('EditLeadsDetails: Navigating with updatedLead', JSON.stringify(updatedLead, null, 2));
...
//       ]);
//     } catch (error) {
//       //.error('EditLeadsDetails: Error updating lead', error);
//       let errorMessage = 'Contact with this email or phone already exists';
...
const EditLeadsDetails = ({ route, navigation }) => {
  ...
  const scrollViewRef = useRef(null);
  const renderCount = useRef(0);
  const hasFetchedDecrypt = useRef(false); // Track if decryption has been fetched
  ...
  // Fetch decrypted data when the screen is focused
  useFocusEffect(
    useCallback(() => {
      if (contactId && !hasFetchedDecrypt.current) {
        console.log(`EditLeadsDetails: Fetching decrypted info for contactId ${contactId}`);
        dispatch(getDecryptInfo(contactId))
          .then((resultAction) => {
            if (getDecryptInfo.fulfilled.match(resultAction)) {
              hasFetchedDecrypt.current = true; // Mark as fetched
              const { phone, email } = resultAction.payload?.data || {};
              if (phone) setMobileNo(phone);
              if (email) setEmail(email);
            }
          })
          .catch((error) => {
            console.error('EditLeadsDetails: Error fetching decrypted info:', error);
            Alert.alert('Error', 'Failed to fetch decrypted contact information.');
          });
      }

      // return () => {
      //   console.log('EditLeadsDetails: Resetting decrypt info');
      //   dispatch(resetDecryptInfo());
      //   hasFetchedDecrypt.current = false; // Reset for next navigation
      // };
    }, [dispatch, contactId])
  );
  ...
  const handleSubmit = async () => {
    if (!leadId || !contactId) {
      console.error('EditLeadsDetails: Invalid Lead or Contact ID', { leadId, contactId });
      Alert.alert('Error', 'Invalid Lead or Contact ID.');
      return;
    }
    if (!source || !sourceId || !isValidObjectId(sourceId)) {
    ...
    >
      <StyledView className="flex-1">
        ...
        >
          ...
          ) : (sourcesError || branchesError || decryptError) ? (
            <StyledView className="mb-4">
              <StyledText className="text-red-500 text-[12px] font-poppins">
                Error: {sourcesError || branchesError || decryptError || 'Failed to load data'}
              </StyledText>
              <CustomButton
                buttonName="Retry"
                onPress={() => {
                  ...
                  dispatch(fetchAllBranches());
                  if (contactId) {
                    hasFetchedDecrypt.current = false; // Allow retry
                    dispatch(getDecryptInfo(contactId));
                  }
                }}
                gradientColors={['#8290EA', '#3F4CA0']}
