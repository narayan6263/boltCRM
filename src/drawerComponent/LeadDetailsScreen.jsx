...
// // import { fetchLeadById, resetLeadByIdState } from '../redux/slice/index';
...
// //   const { lead, previousScreen } = route.params || {};
...
// //               left: index * screenWidth, // Position each tab correctly
...
// // LeadDetailsScreen.jsx
// import React, { useEffect, useState, useCallback, useRef } from 'react';
// import { View, ActivityIndicator, Text, TouchableOpacity, ScrollView, Animated, Dimensions } from 'react-native';
// import { styled } from 'nativewind';
// import { useDispatch, useSelector } from 'react-redux';
// import { fetchLeadById, resetLeadByIdState } from '../redux/slice/index';
// import { format } from 'date-fns';
...
//   // Fetch lead details only once when component mounts
//   useEffect(() => {
//     if (leadId) {
//       console.log(`LeadDetailsScreen: Fetching details for leadId ${leadId}`);
//       dispatch(fetchLeadById(leadId));
...
//       dispatch(resetLeadByIdState());
//     };
//   }, [dispatch, leadId]); // Dependencies ensure this runs only when leadId or dispatch changes
...
const LeadDetailsScreen = ({ navigation, route }) => {
  ...
  const [activeTab, setActiveTab] = useState(0);
  const [renderCount, setRenderCount] = useState(0);
  const hasFetchedLead = useRef({}); // Track fetched leadIds
  ...
  useEffect(() => {
    if (leadId && !hasFetchedLead.current[leadId]) {
      console.log(`LeadDetailsScreen: Fetching details for leadId ${leadId}`);
      dispatch(fetchLeadById(leadId))
        .then(() => {
          hasFetchedLead.current[leadId] = true; // Mark as fetched
        })
        .catch((err) => console.error('LeadDetailsScreen: Dispatch error:', err));
    } else if (leadId && hasFetchedLead.current[leadId]) {
      console.log(`LeadDetailsScreen: Skipping fetch for leadId ${leadId}, using cached data`);
    }
      ...
      {error && (
        <StyledView className="flex-1 justify-center items-center">
          ...
          <StyledTouchable
            className="bg-blue-500 p-2 rounded-md mt-4"
            onPress={() => {
              hasFetchedLead.current[leadId] = false; // Allow retry
              dispatch(fetchLeadById(leadId));
            }}
          >
            <StyledText className="text-white">Retry</StyledText>
