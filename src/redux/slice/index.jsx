...
export const fetchLeadsByFollowUpDate = createAsyncThunk(
  ...
  async ({ queryType, page = 1, pageSize = 10, isLoadMore = false }, { rejectWithValue }) => {
    try {
      const response = await apiService.searchLeadsByFollowUpDate(queryType, page, pageSize);
      if (!response.data) throw new Error('Invalid API response: Missing data');
      const { leads, totalLeads, currentPage, totalPages } = response.data;
      if (!Array.isArray(leads)) throw new Error('Invalid API response: leads is not an array');
      ...
      return { leads: filteredLeads, totalLeads, currentPage, totalPages };
    } catch (error) {
...
const searchLeadsByFollowUpDateSlice = createSlice({
  ...
  initialState,
  reducers: {
    resetSearchLeadsState: (state, action) => {
      const queryType = action.payload || 'pending';
    ...
    setLeadsFromProps: (state, action) => {
      const { queryType, leads, totalLeads, currentPage, totalPages } = action.payload;
      state.data[queryType] = {
        leads: leads || [],
  ...
  extraReducers: (builder) => {
    builder
      .addCase(fetchLeadsByFollowUpDate.pending, (state, action) => {
        state.loading = true;
        state.error = null;
      })
      .addCase(fetchLeadsByFollowUpDate.fulfilled, (state, action) => {
        ...
        state.data[queryType] = {
          leads: isLoadMore
            ? [...state.data[queryType].leads, ...action.payload.leads]
