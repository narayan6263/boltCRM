...
//     try {
//       console.log(`🌐 Calling searchLeadsByFollowUpDate with queryType: ${queryType}, page: ${page}, pageSize: ${pageSize}`);
//       const orgId = await AsyncStorage.getItem('orgId');
//       if (!orgId) {
//         throw new Error('Organization UUID not found in storage');
//       }
//       const response = await apiClient.get(ENDPOINTS.SEARCH_LEADS_BY_FOLLOW_UP, {
//         params: { queryType, page, pageSize },
//         headers: { Org_uuid: orgId },
//       });
//       console.log('✅ searchLeadsByFollowUpDate API Response:', response.data);
//       return response.data;
//     } catch (error) {
//       console.error(
//         '❌ Error searching leads by follow-up date:',
...
const apiService = {
  ...
  async getEmployeeById(id) {
    try {
      ...
      const response = await apiClient.get(ENDPOINTS.GET_EMPLOYEE_BY_ID(id));
      console.log('✅ Fetched Employee:sasa', response.data.data);
      return response.data.data;
    } catch (error) {
      console.error('❌ Error fetching employee:', error.response?.data || error.message);
      throw handleError(error);
