...
// import { fetchLeadsByFollowUpDate, resetSearchLeadsState, getDecryptInfo, resetDecryptInfo } from '../redux/slice/index';
...
//   const { leads = [], loading, error, totalLeads, currentPage, totalPages } = useAppSelector(
//     (state) => state.searchLeadsByFollowUpDate || {}
//   );
//   const { queryType = 'pending' } = route.params || {};
//   const { data: decryptedData, status: decryptStatus, error: decryptError } = useAppSelector((state) => state.decryptInfo);
...
//   const fetchDecryptedPhone = useCallback(async (contactId) => {
//     if (decryptedPhoneNumber) {
//       return decryptedPhoneNumber;
//     }

//     if (!contactId) {
//       Alert.alert('Error', 'No contact ID provided for decryption.');
//       return null;
//     }

//     try {
//       setIsLoadingPhone(true);
//       const result = await dispatch(getDecryptInfo(contactId)).unwrap();
//       const phone = result?.data?.phone || result?.phone;
//       if (phone) {
//         setDecryptedPhoneNumber(phone);
//         return phone;
//       } else {
//         Alert.alert('Error', 'Decrypted phone number not found in response.');
//         return null;
//       }
//     } catch (err) {
//       console.error('FollowUps: Error fetching decrypted phone', err);
//       Alert.alert('Error', 'Failed to decrypt phone number.');
//       return null;
//     } finally {
//       setIsLoadingPhone(false);
//     }
//   }, [dispatch, decryptedPhoneNumber]);

//   const handleCall = async (phone, leadId, contactId) => {
...
//     if (contactId) {
//       setIsLoadingPhone(true);
//       const decrypted = await fetchDecryptedPhone(contactId);
//       setIsLoadingPhone(false);
//       if (decrypted) {
//         phoneToUse = decrypted;
//       } else {
//         Alert.alert('Error', 'Failed to get decrypted phone number.');
//         return;
//       }
//     }
...
//     const formattedPhone = phoneToUse.startsWith('+') ? phoneToUse : `+91${phoneToUse.replace(/[\\s\\-\\(\\)]/g, '')}`;
...
//       setDecryptedPhoneNumber(null);
//     }
//   };

//   const handleWhatsApp = async (phone, contactId) => {
//     let phoneToUse = phone;
//     if (contactId) {
//       setIsLoadingPhone(true);
//       const decrypted = await fetchDecryptedPhone(contactId);
...
//   const handleSMS = async (phone, contactId) => {
//     let phoneToUse = phone;
//     if (contactId) {
//       setIsLoadingPhone(true);
//       const decrypted = await fetchDecryptedPhone(contactId);
...
//     // Determine the previous screen based on queryType
//     let previousScreenName;
//     if (queryType === 'today') {
//       previousScreenName = 'TodayFollowUps';
//     } else if (queryType === 'tomorrow') {
//       previousScreenName = 'TomorrowFollowUps';
//     } else {
//       // Default for 'pending' or any other queryType
//       previousScreenName = 'PendingFollowUps';
//     }

//     try {
//       navigation.navigate('LeadDetailsScreen', {
//         lead,
//         previousScreen: previousScreenName // Pass the determined previous screen
//       });
//     } catch (error) {
...
//             {[
//               { icon: require('../assets/call.png'), label: 'Call', onPress: () => handleCall(item.phone, item.leadId, item.contactId) },
//               { icon: require('../assets/whatsapp.png'), label: 'Whatsapp', onPress: () => handleWhatsApp(item.phone, item.contactId) },
//               { icon: require('../assets/sms.png'), label: 'SMS', onPress: () => handleSMS(item.phone, item.contactId) },
...
import Ionicons from 'react-native-vector-icons/Ionicons';
import { useAppDispatch, useAppSelector } from '../hooks/useRedux';
import { fetchLeadsByFollowUpDate, getDecryptInfo, resetDecryptInfo, setLeadsFromProps } from '../redux/slice/index';
import { useNavigation } from '@react-navigation/native';
import LinearGradient from 'react-native-linear-gradient';
import { RFPercentage } from 'react-native-responsive-fontsize';
import Header from '../component/Header';
...
const FollowUps = ({ route }) => {
  const dispatch = useAppDispatch();
  const navigation = useNavigation();
  const { queryType = 'pending', leads: propsLeads, totalLeads: propsTotalLeads, currentPage: propsCurrentPage, totalPages: propsTotalPages } = route.params || {};
  const { data, loading, error } = useAppSelector((state) => state.searchLeadsByFollowUpDate || {});
  const { leads = [], totalLeads = 0, currentPage = 1, totalPages = 1 } = data[queryType] || {};
  ...
  useEffect(() => {
    if (queryType === 'today' && propsLeads?.length > 0) {
      dispatch(
        setLeadsFromProps({
          queryType,
          leads: propsLeads,
          totalLeads: propsTotalLeads || propsLeads.length,
          currentPage: propsCurrentPage || 1,
          totalPages: propsTotalPages || 1,
        })
      );
    } else if (!leads.length && !propsLeads?.length) {
      dispatch(fetchLeadsByFollowUpDate({ queryType, page: 1, pageSize: 10 }));
    }
  }, [dispatch, queryType, leads.length, propsLeads, propsTotalLeads, propsCurrentPage, totalPages]);
  ...
  const truncateText = (text) => {
    if (!text || typeof text !== 'string') return { text: 'N/A', hasMore: false };
    const words = text.trim().split(' ');
    return words.length > 1 ? { text: words[0], hasMore: true } : { text: words[0] || 'N/A', hasMore: false };
  };
  ...
  const fetchDecryptedPhone = useCallback(
    async (contactId) => {
      if (decryptedPhoneNumber) return decryptedPhoneNumber;
      if (!contactId) {
      ...
      try {
        ...
        Alert.alert('Error', 'Decrypted phone number not found.');
        return null;
      } catch {
        Alert.alert('Error', 'Failed to decrypt phone number.');
        return null;
  ...
  const handleCall = async (phone, leadId, contactId) => {
    ...
    let phoneToUse = phone;
    if (contactId) {
      phoneToUse = await fetchDecryptedPhone(contactId);
    ...
    try {
      await Linking.openURL(telUrl);
      const sourceScreen = queryType === 'today' ? 'TodayFollowUps' : queryType === 'tomorrow' ? 'TomorrowFollowUps' : 'PendingFollowUps';
      navigation.navigate('FeedbackScreen', { phoneNumber: phoneToUse, leadId, sourceScreen, queryType });
    } catch {
      Alert.alert('Error', 'Failed to make call.');
    } finally {
      dispatch(resetDecryptInfo());
  ...
  const handleLeadPress = (lead) => {
    if (!lead.leadId || !lead.name || !lead.phone) {
      Alert.alert('Error', 'Incomplete lead data.');
      return;
    }
    const sourceScreen = queryType === 'today' ? 'TodayFollowUps' : queryType === 'tomorrow' ? 'TomorrowFollowUps' : 'PendingFollowUps';
    try {
      navigation.navigate('LeadDetailsScreen', { lead, previousScreen: sourceScreen });
    } catch {
      Alert.alert('Error', 'Failed to navigate to lead details.');
    }
  };
  ...
  const renderItem = ({ item }) => (
    <TouchableOpacity onPress={() => handleLeadPress(item)}>
      <StyledView className="bg-white rounded-md shadow-sm mb-4 mx-4 border border-[#C8C8C8]">
        <StyledView className="w-full space-y-2 ml-4 p-3">
          <View className="flex-row">
            <View className="w-[195] flex-row items-center gap-x-6">
              <StyledText className="text-subHeading font-familyFontLatoBold text-[#000]">
            ...
            <View className="w-1/2 flex-row items-center gap-x-2">
              <Image source={require('../assets/modal.png')} className="w-[15px] h-[15px]" resizeMode="contain" />
              <StyledText className="text-[#000] text-inputs font-familyFontInter">
                {truncateText(item.model).text}
              </StyledText>
              {truncateText(item.model).hasMore && (
          ...
          <View className="flex-row">
            <View className="w-[195] flex-row items-center gap-x-6">
              <Image source={require('../assets/personLead.png')} className="w-[15px] h-[15px]" resizeMode="contain" />
              <StyledText className="text-[#000] text-inputs font-familyFontInter">
                {item.assignedTo || 'N/A'}
              </StyledText>
            </View>
            <View className="w-1/2 flex-row items-center gap-x-2">
              <Image source={require('../assets/freshLead.png')} className="w-[15px] h-[15px]" resizeMode="contain" />
              <StyledText className="text-inputs font-familyFontInter" style={{ color: item.statusColor || '#4DD041' }}>
                {item.status || 'N/A'}
              </StyledText>
            </View>
          </View>
          <View className="flex-row">
            <View className="w-[195] flex-row items-center gap-x-6">
              <Image source={require('../assets/floorvisit.png')} className="w-[15px] h-[15px]" resizeMode="contain" />
              <StyledText className="text-[#000] text-inputs font-familyFontInter">
                {truncateText(item.source).text}
              </StyledText>
              {truncateText(item.source).hasMore && (
            ...
            <View className="w-1/2 flex-row items-center gap-x-2">
              <Image source={require('../assets/calender.png')} className="w-[15px] h-[15px]" resizeMode="contain" />
              <StyledText className="text-[#000] text-inputs font-familyFontInter">
                {formatDate(item.nextFollowUpDate)}
              </StyledText>
            </View>
  ...
  const getHeaderTitle = () => {
    switch (queryType) {
      case 'today': return 'Today FollowUps Detail';
      case 'tomorrow': return 'Tomorrow FollowUps Detail';
      default: return 'Pending FollowUps Detail';
    }
  };
  ...
  return (
    <SafeAreaView className="flex-1 bg-white">
      ...
      <StyledView className="flex-1 bg-white">
        ...
        {loading && !leads.length && !propsLeads?.length ? (
          <StyledText className="text-black text-center mt-5">Loading...</StyledText>
        ) : error ? (
          <StyledText className="text-red-500 text-center mt-5">Error: {error}</StyledText>
        ) : (
