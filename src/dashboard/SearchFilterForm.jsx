...
const SearchFilterForm = ({ navigation, onClose, onSearchSubmitted }) => {
  ...
  const { activeSource = [], loading: sourcesLoading, error: sourcesError } = useSelector((state) => state.activeSource || {});
  const { activeEmployees = [], loading: employeesLoading, error: employeesError } = useSelector((state) => state.activeEmployees || {});
  const { activeBranches = [], loading: branchesLoading, error: branchesError } = useSelector((state) => state.activeBranches || {});
  const { statuses = [], loading: statusesLoading, error: statusesError } = useSelector((state) => state.activeStatuses || {});
  ...
  const priorities = ['Lower', 'Mid', 'Important', 'High Priority and Urgent'];
    ...
    <StyledView className="bg-white rounded-b-2xl shadow-lg">
      <Header
        ...
        containerStyle={{
          ...
          borderBottomColor: '#E0E0E0', // Consistent with HomeScreen and MapScreen
        }}
      />
